...
def encode(n: int, nof_colors: int, break_symmetries: bool) -> CNF:
    ...
    for i in range(1, n + 1):
        ...
        for c in range(1, nof_colors + 1):
            for cc in range(c + 1, nof_colors + 1):
                cnf.append([-v_cnf(i, c), -v_cnf(i, cc)])
    for c in range(1, nof_colors + 1):
        for i in range(1, n + 1):
            for ii in range(1, n + 1):
                if i + ii <= n:
                    cnf.append([-v_cnf(i, c), -v_cnf(ii, c), -v_cnf(i+ii, c)])
    ...
    if break_symmetries:
        ...
        if nof_colors == 4 and n >= 3:
            for i in range (3, n+1):
                clauses = []
                for ii in range(3, i-1+1):
                    clauses.append(v_cnf(ii, 3))
                clauses.append(-v_cnf(i, 4))
                cnf.append(clauses)
    return cnf
