...
### YOUR # CODE # HERE ###
### YOUR # CODE # HERE ###
    return BDDEXIST(bdd,newvars,BDDAND(bdd,transbdd,BDDRENAME(bdd,old2new,self.subformula.modelCheck(bdd,transdata))))
...
### YOUR # CODE # HERE ###
### YOUR # CODE # HERE ###
    phibdd1 = self.subformula1.modelCheck(bdd,transdata)
    phibdd2 = self.subformula2.modelCheck(bdd,transdata)
    S = {}
    S[0] = phibdd2
    i = 0
    while True:
      S[i+1] = BDDOR(bdd, phibdd2, BDDAND(bdd,phibdd1,BDDEXIST(bdd,newvars,BDDAND(bdd,transbdd,BDDRENAME(bdd,old2new,S[i])))))
      print("Computation of EG: BDD of size " + str(len(S[i+1])))
      i = i + 1
      if S[i] == S[i-1]:
        print("EG fixpoint at iteration " + str(i) + " for " + str(self))
