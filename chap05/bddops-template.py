...
#### ADD YOUR MODEL-COUNTING CODE HERE

        # Do model-count
        if isinstance(child1, int):
            if child1 == 1:
                count1 = round(math.pow(2, len(self.vars) - self.varIndex[rootVar] - 1))
            else:
                count1 = 0
        else:
            count1 = round(math.pow(2, self.varIndex[child1.nodeVar] - self.varIndex[rootVar] - 1)) * child1.modelCount



        if isinstance(child2, int):
            if child2 == 1:
                count2 = round(math.pow(2, len(self.vars) - self.varIndex[rootVar] - 1))
            else:
                count2 = 0
        else:
            count2 = round(math.pow(2, self.varIndex[child2.nodeVar] - self.varIndex[rootVar] - 1)) * child2.modelCount



        # Store model-count

        newNode.modelCount = count1 + count2
